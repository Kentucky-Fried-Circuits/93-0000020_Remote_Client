...
class Instrument:
    ...
    Args:
        ...
        * timeout (float): read timeout in seconds for serial write
    ...
    def _communicate(self, request, number_of_bytes_to_read):
        ...
        if not answer:
            raise NoResponseError(f'No communication with the instrument (null answer)')
...
def _extract_payload(response, slaveaddress, mode, functioncode):
    ...
    * ASCII Mode: header (:) + slaveaddress byte + functioncode byte +
    ...
    if mode != MODE_ASCII:
        if ord(response[0]) == 1 and ord(response[1]) >= 0x80: #Exception Reponse, for some reason we're receiving 2 extra bytes
            if len(response) > 5:
...
def _check_response_slaveerrorcode(response):
    ...
    NON_ERRORS = [5]
    SLAVE_ERRORS = {
        1: IllegalRequestError("Server reported illegal function"),
        2: IllegalRequestError("Server reported illegal data address"),
        3: IllegalRequestError("Server reported illegal data value"),
        4: SlaveReportedException("Server reported device failure"),
        6: SlaveDeviceBusyError("Server reported device busy"),
        7: NegativeAcknowledgeError("Server reported negative acknowledge"),
        8: SlaveReportedException("Server reported memory parity error"),
        10: SlaveReportedException("Server reported gateway path unavailable"),
        11: SlaveReportedException(
            "Server reported gateway target device failed to respond"
        ),
    }
    ...
    if _check_bit(received_functioncode, _BITNUMBER_FUNCTIONCODE_ERRORINDICATION):
        ...
        error = SLAVE_ERRORS.get(
            ...
            SlaveReportedException(
                "Server reported error code " + str(slave_error_code)
            ),
        )
        raise error
...
#         output += "Server address: {} (dec). Function code: {} (dec).\n".format(
...
#         )  # Server address or function code not available
