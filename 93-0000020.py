...
port = 'COM5'  # this is ignored, and the USB VID is used instead
baud = 57600
debug = False
address = 1
...
regs = {
    ...
    "PID_SET_POINT":				61,  # pid set point current * 256 (the minimum of available_current, MAX_CHARGE_CURRENT, and charging_current)
...
# reverse dictionary for register names
reg_names = {}
...
#    while True:
    if values['-LOGGING-'] == True and connected:
        try:
            ...
            print(f'Unexpected error: {e}. Continuing on...')
        else:
...
#                 wait -= 0.001
#             else:
#                 # there's no data, so increase the wait time. If we increase the same amount we decrease, we'll end up reading an empty log 50% of the time
#                 wait += 0.005
...
def connect_port():
    ...
    if len(ports) == 0:
        proceed = sg.popup_ok("No serial port found")
        return None
    ...
    for port, desc, hwid in sorted(ports):
        ...
        if "USB Serial Port" in desc:
            found_port = port
        ret += ("Found generic USB serial at " + found_port)
        ret += '\n'
    ...
    if found_port == "":
        ...
        try:
            ...
            instrument = minimalmodbus.Instrument(found_port, address, baudrate=baud, debug=debug,
...
# MAIN
def main():
    global instrument, log_interval, window, log_handle
    ...
    top = [
            [sg.B("Connect", key='-CONNECT-', disabled=not(connected)), sg.B("CAN", key='-CAN-', disabled=False)],
